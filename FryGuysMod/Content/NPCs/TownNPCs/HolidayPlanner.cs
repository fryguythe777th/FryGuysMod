...
namespace FryGuysMod.Content.NPCs.TownNPCs
{
	...
	public class HolidayPlanner : ModNPC
	{
		public override void SetStaticDefaults()
		{
			...
			NPC.Happiness.LoveBiome(ModContent.GetInstance<TechnicalGraveyardBiome>().Type);
			NPC.Happiness.LikeBiome(ModContent.GetInstance<TechnicalHellBiome>().Type);

			NPC.Happiness.LoveNPC(NPCID.SantaClaus);
			...
			if (!firstButton)
            {
				for (int i = 10; i < 50; i++)
                {
					...
					if (FryGlobalItem.Sets.IsHolidayItem[item.type] && !item.favorited && !item.IsAir)
                    {
						...
						if (Coin[0] > 0)
							Item.NewItem(Main.LocalPlayer.getRect(), ItemID.CopperCoin, Coin[0]); //the number inside Coin[] refers to the type of coin. 0 = copper, 1 = silver, on until 3. You HAVE to do this for each type of coin

						if (Coin[1] > 0)
							Item.NewItem(Main.LocalPlayer.getRect(), ItemID.SilverCoin, Coin[1]);

						if (Coin[2] > 0)
							Item.NewItem(Main.LocalPlayer.getRect(), ItemID.GoldCoin, Coin[2]);

						if (Coin[3] > 0)
							Item.NewItem(Main.LocalPlayer.getRect(), ItemID.PlatinumCoin, Coin[3]);
